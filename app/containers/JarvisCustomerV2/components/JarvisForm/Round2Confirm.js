...
const schema = yup.object().shape({
  confirmValidateDocs: yup.string().required('Bạn chưa xác nhận'),
});
...
export default function Round2Confirm(props) {
  ...
  return (
    <JarvisFormStyle>
      ...
      <div className="roundName2">CHỨNG MINH THU NHẬP & KIỂM TRA HẠN MỨC</div>
      <form onSubmit={handleSubmit(onSubmitForm)}>
        ...
        <div className="virtualcardConfirm">
          Bạn có đảm bảo cung cấp được sao kê chứng minh thu nhập như yêu cầu
          không?
        </div>
          ...
          <div>
            {errors.confirmValidateDocs && (
              <span className="formError">
                {errors.confirmValidateDocs.message}
              </span>
            )}
          </div>
