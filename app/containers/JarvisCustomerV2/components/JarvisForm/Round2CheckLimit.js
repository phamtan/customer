	...
	return (
		<JarvisFormStyle>
			...
			<div className="roundNameDescription">
				Việc kiểm tra thông tin sẽ giúp bạn hoàn thành hồ sơ nhanh hơn. Quá trình này sẽ diễn ra trong vài phút
			</div>
			...
			{checkLimit < 600 &&
				<div className="checkLimitContainer">
					<div className="waitingTitle"><span>Đang chờ tới lượt {checkLimit}/600</span>
						<Spinner animation="border" size="sm" variant="success" role="status">
							<span className="sr-only">Loading...</span>
						</Spinner>
					</div>
					<div className="waitingDescription">Chúng tôi đang xử lý nhiều hồ sơ cùng lúc, Kết quả kiểm tra sẽ thông tin sẽ thông tin đến bạn ngay</div>
					<div className="waitingNote">
						Nếu bạn tắt trình duyệt lúc này sẽ phải đăng nhập lại
		. Tuy nhiên bạn vẫn có thể sử dụng các ứng dụng khác trong thời gian chờ. <span onClick={() => props.setStep(999)}>Thông tin đến bạn!</span></div>
				</div>
			}
			{checkLimit > 600 &&
				<>
					...
					<button onClick={() => props.setStep(999)} type="button" className="btn btnSubmit">
						Tiếp tục
        			</button>
