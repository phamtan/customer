...
const schema = yup.object().shape({
	creditLimit: yup
		.string()
		.required('Bạn chưa nhập hạn mức'),
});
...
export default function RegisLimitInfo(props) {
	...
	return (
		<JarvisFormStyle>
			...
			<div className="roundName">CHỌN HẠN MỨC TÍN DỤNG</div>
			...
			<form className="formWrapper" onSubmit={handleSubmit(onSubmitForm)}>
				<div className="virtualcardConfirm">
					{_.get(jarvisCustomer, "selectedCard.name", "")} có hạn mức tối đa là {_.get(jarvisCustomer, "selectedCard.limit", "").toLocaleString()} VNĐ
				</div>
				<div className="formWrapper">
					...
					<Controller
						...
						control={control}
						render={props => (
							<RadioGroup aria-label="gender" name="limitType" value={props.value} >
								<FormControlLabel value="1" control={<Radio />} label="Sử dụng hạn mức tối đa" onChange={props.onChange} />
								{props.value === "1" &&
									<div className="form-group">
										...
										/>
										{errors.creditLimit && (
											<span className="formError">{errors.creditLimit.message}</span>
										)}
									</div>
								}
								<FormControlLabel value="2" control={<Radio />} label="Tôi muốn giảm hạn mức" onChange={props.onChange} />
								{props.value === "2" &&
									<div className="form-group">
